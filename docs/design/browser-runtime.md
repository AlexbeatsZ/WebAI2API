...
`GET /health` exposes only readiness, config version, uptime, safe-mode state, and aggregate capacity. `GET /admin/diagnostics` is the authenticated maintenance snapshot. A site check atomically reserves one page in an exact profile/site pair, brings it to the front for profile VNC, and observes its sanitized URL, title, model-cache state, and non-value interactive attributes without generating content. It never returns input values, page text, cookies, or storage. A real probe also targets one exact profile/site pair and never fails over to another profile, so proxy and account acceptance results stay attributable.
